...
use crate::code::runtime::node::{NodeConditionalImport, NodeConditionalImportKind, NodeConfig};
...
fn test_parse_source_file_parses_es_package_dependency_import_without_subpath() {
    for path in &["lambda.js", "lambda.mjs"] {
        ...
        let mut runtime_config: RuntimeConfig = RuntimeConfig::default();
        let mut node_config: NodeConfig = Default::default();
        node_config.dependencies.push("db-dep".to_string());
        runtime_config.set_node_config(node_config);
        let module = parse_source_file(
            project_test.source_path(path),
            Arc::new(Mutex::new(runtime_config)),
        )
        .unwrap();
...
fn test_parse_source_file_parses_es_node_subpath_import_as_unknown() {
    for path in &["lambda.js", "lambda.mjs"] {
        ...
        let mut runtime_config: RuntimeConfig = RuntimeConfig::default();
        let mut node_config: NodeConfig = Default::default();
        node_config.subpath_imports.insert(
            "#db".to_string(),
...
#[ignore]
#[test]
fn test_parse_source_file_parses_es_export_fn_assignment() {
    for path in &["lambda.js", "lambda.mjs"] {
...
#[ignore]
#[test]
fn test_parse_source_file_parses_ts_export_fn_assignment() {
    let path = "lambda.ts";
    let project_test =
