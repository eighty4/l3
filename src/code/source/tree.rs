...
pub struct SourceTree {
    ...
    project_details: Arc<ProjectDetails>,
    pub sources: HashMap<PathBuf, Arc<SourceFile>>,
}

impl SourceTree {
    pub fn new(project_details: Arc<ProjectDetails>) -> Self {
        Self {
    ...
    pub async fn initialize(&mut self) -> Result<(), anyhow::Error> {
        ...
        for path in
            parallel::recursively_read_dirs(&self.project_details.dir.join("routes")).await?
        {
            ...
            let project_details = self.project_details.clone();
            parse_route_sources.spawn(async move {
                let source_path = SourcePath::from_abs(&project_details.dir, path);
                parse_source_file(source_path.clone(), &project_details)
            });
        }
            ...
            parse_lambda_fns.spawn(async move {
                ...
                if handler_fns.is_empty() {
                    ...
                    for (http_method, handler_fn) in handler_fns {
                        ...
                        let env_var_sources = EnvVarSources::new(&project_details.dir, &route_key)?;
                        lambdas.push(LambdaFn::new(
                            env_var_sources,
                            handler_fn,
                            source_file.path.clone(),
                            &project_details.name.clone(),
                            route_key,
                        ));
                    }
                }
